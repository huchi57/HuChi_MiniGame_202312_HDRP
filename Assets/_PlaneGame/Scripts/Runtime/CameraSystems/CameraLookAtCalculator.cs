...
namespace UrbanFox.MiniGame
{
    public class CameraLookAtCalculator : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (m_lookAtWeights.IsNullOrEmpty() || !GameStateManager.PlayerTransform)
            {
                m_weightedLookAt = GameStateManager.PlayerTransform;
                return;
            }
            ...
            m_positionSum = (m_weightOfPlayer / m_weightSum) * GameStateManager.PlayerTransform.position;
            foreach (var data in m_lookAtWeights)
            {
